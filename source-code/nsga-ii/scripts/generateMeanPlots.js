  ...
  .forEach((directory) => {
    ...
    files
      ...
      .forEach((file) => {
        ...
        //   generationsData[index].total += generation[0][0];
        //   generationsData[index].elements++;
        // });
        parsedData.forEach((individualData, index) => {
          if (individualData[1] === 0) {
            const generationIndex = Math.floor(index / 100);
            generationsData[generationIndex].total1 += individualData[0];
            generationsData[generationIndex].elements1++;
          }
        });
      });
    ...
    files
      ...
      .forEach((file) => {
        ...
        parsedData.forEach((individualData, index) => {
          if (individualData[1] === 0) {
            const generationIndex = Math.floor(index / 100);
            generationsData[generationIndex].total2 += individualData[0];
            generationsData[generationIndex].elements2++;
          }
        });
      });
