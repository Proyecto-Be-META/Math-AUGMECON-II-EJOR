...
param D;
param puntos; #Sirve para determinar los puntos de las cotas obtenidos por el heurístico multi objetivo
...
param d{clientes};
param cotas{Fsize,Obj};#Sirve para leer los puntos de las cotas 
...
minimize Biobjetivo1:
sum{i in origenes, j in clientes: j>0} o[i,j] * (sum{k in niveles} (k*w[i,j,k])) + sum{i in clientes, j in clientes: 0<i<>j>0} cost[i,j]*(sum{k in niveles: k < N} (k*y[i,j,k])) - e*(SL/r1);

minimize Biobjetivo2:
sum{i in origenes, j in clientes: j>0} o[i,j] * (sum{k in niveles} (w[i,j,k])) + sum{i in clientes, j in clientes: 0<i<>j>0} cost[i,j]*(sum{k in niveles: k < N} (y[i,j,k])) - e*(SD/r2);

#*****************************************************
#  	          Restricciones AUGMECON	             *
#*****************************************************

subject to latencia_total_aug:
sum{i in origenes, j in clientes: j>0} o[i,j] * (sum{k in niveles} k*(w[i,j,k])) + sum{i in clientes, j in clientes: 0<i<>j>0} cost[i,j]*(sum{k in niveles: k < N} k*(y[i,j,k])) + SL = eps1;

subject to distancia_total_aug:
sum{i in origenes, j in clientes: j>0} o[i,j] * (sum{k in niveles} (w[i,j,k])) + sum{i in clientes, j in clientes: 0<i<>j>0} cost[i,j]*(sum{k in niveles: k < N} (y[i,j,k])) + SD = eps2;
